...
import { FaUsers, FaSearch, FaEye, FaPrint, FaSpinner, FaUserCircle, FaCheckCircle, FaClock } from 'react-icons/fa';
import { getMembers, approveMember } from '../../api/memberApi';
...
const FamilyMembersPage = () => {
    ...
    const fetchMembers = async () => {
        try {
            ...
            if (response.success) {
                ...
                setStats({ total, male, female, pending });
            }
        } catch (error) {
    ...
    const handleApprove = async (id) => {
        try {
            const response = await approveMember(id);
            if (response.success) {
                        ...
                        <tbody>
                            {members.map((member, index) => (
                                <tr key={member.member_id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="py-3 px-4">
                                        <span className={`text-xs px-2 py-1 rounded-full ${
                                            member.status?.toLowerCase() === 'active' ? 'bg-green-100 text-green-600' : 
                                            member.status?.toLowerCase() === 'pending' ? 'bg-yellow-100 text-yellow-600' :
                                        ...
                                        <div className="flex items-center gap-2">
                                            {member.status?.toLowerCase() === 'pending' && (
                                                <button 
                                                    ...
                                                    className="p-1.5 bg-green-50 text-green-500 rounded hover:bg-green-100 transition-colors" 
                ...
                {members.length === 0 && (
                    <div className="text-center py-12">
                        ...
                        <p className="text-gray-400 text-sm">
                            {search ? 'No members match your search criteria.' : 'No members in your family yet.'}
                        </p>
                    </div>
                )}
            </div>
